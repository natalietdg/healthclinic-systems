...
const ImageUpload: React.FC<ImageUploadProps> = ({ blob, name, multiple=false, onChangeImg }) => {
    ...
    return(
        <div className="image-upload">
            <div style={{display: 'flex', flexDirection: 'row'}}>
                {
                    multiple == true && imgArray.map((tempUrl, index)=> {
                        // console.log('tempUrl', tempUrl);
                        return <span>
                            <img src={tempUrl} />
                            <button name = {index.toString()} style={{background: 'rgb(0,0,0,0)', border: '0px solid grey', position: 'absolute', top: 'initial', right: 'initial'}} onClick={ (e) => removePic(e)} ><img style={{width: '20px', height: '20px', padding: '0px', margin: '0px', boxShadow: 'none'}} src="/assets/images/close-window.png" /></button>
                        </span>
                    })
                }
            </div>
            ...
            <label className="image-upload--input" >
                <input hidden multiple={multiple} type="file" accept="image/png, image/jpeg" onChange={(e) => handleChange(e)}/>
                Choose File
            </label>
        </div>
    ) 
